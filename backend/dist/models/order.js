...
class OrderStore {
    ...
    async create(usr, status) {
        try {
            ...
            throw new Error(`Could not create order for user ${usr.id}. Error: ${err}`);
        }
    }
    async delete(id) {
        try {
            const conn = await database_1.default.connect();
            //SERIAL PRIMARY KEY? figure out how to factor that in
            let sql = 'DELETE FROM order_products WHERE order_products.order_id=($1)';
    ...
    async addProduct(quantity, orderID, productID) {
        try {
            ...
            let sql = 'INSERT INTO order_products (quantity, order_id, product_id) VALUES( $1, $2, $3 );';
    ...
    async getOrders(o) {
        try {
            ...
            const result = await conn.query(
                sql, [ o.id ] );*/
            const query = {
                text: 'SELECT product_id, quantity FROM products INNER JOIN order_products ON order_products.product_id = products.id WHERE order_products.order_id = $1;',
                values: [o.id],
                rowMode: 'array',
            };
            ...
            throw new Error(`Could not get the products in order ${o.id}: ${err}`);
        }
    }
